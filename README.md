...
目录
-----
- [ECUnionSDK接入文档](#ecunionsdk----)
  * [目录](#--)
  * [接入方只需要接好接口，SDK要我方二次打包后才会生效。](#-----------sdk-------------)
    + [1.说明](#1--)
    + [2.搭建开发环境](#2------)
    + [3.1 SDK的Application初始化（必接）](#31-sdk-application-------)
    + [3.2 SDK主activity初始化（必接）](#32-sdk-activity-------)
    + [3.3 开屏广告](#33-----)
      - [3.3.1开屏接入方式一：自定义Activity继承ECSplashActivity来展示闪屏。推荐此方式接入闪屏！](#331-----------activity--ecsplashactivity----------------)
      - [3.3.2开屏接入方式二：原生activity中调用闪屏接口展示闪屏](#332----------activity-----------)
    + [3.4 广告接口](#34-----)
      - [3.4.1 广告类型说明](#341-------)
    + [3.5 广告回调接口说明](#35---------)
    + [3.6 激励视频服务器发奖励接口说明（使用loadAndShowAd接口才接入）](#36------------------loadandshowad------)
    + [4 退出接口（必接）](#4---------)
    + [5 登录接口（必接）](#5---------)
    + [6 支付接口（选接）](#6---------)
    + [7  补单接口（如果有支付必接）](#7---------------)
    + [9  上报用户游戏信息接口（如有要求，请接入）](#9----------------------)
    + [10  自定义计数事件统计接口（选接）](#10-----------------)
    + [11  自定义计算事件统计接口（选接）](#11-----------------)
